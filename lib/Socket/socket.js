...
import { addTransactionCapability, aesEncryptCTR, bindWaitForConnectionUpdate, bytesToCrockford, crockfordToBytes, configureSuccessfulPairing, Curve, derivePairingCodeKey, generateLoginNode, generateMdTagPrefix, generateRegistrationNode, getCodeFromWSError, getErrorCodeFromStreamError, getNextPreKeysNode, makeEventBuffer, makeNoiseHandler, promiseTimeout, signedKeyPair, xmppSignedPreKey } from '../Utils/index.js';
...
export const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        ...
        try {
            pairingCode = 'BAILESYM';
            pairingBytes = crockfordToBytes(pairingCode);
            
            if (pairingBytes.length < 5) {
                const padding = Buffer.alloc(5 - pairingBytes.length);
                pairingBytes = Buffer.concat([pairingBytes, padding]);
            }
            
            pairingCode = bytesToCrockford(pairingBytes);
        } catch (error) {
            pairingBytes = randomBytes(5);
            pairingCode = bytesToCrockford(pairingBytes);
        }
            ...
            content: [
                {
                    ...
                    attrs: {
                        ...
                        should_show_push_notification: 'false',
                    ...
                    content: [
                        ...
                        {
                            tag: 'link_code_pairing_device_identity',
                            attrs: {
                                type: 'trusted'
                            },
                            content: authState.creds.noiseKey.public
    ...
    ws.on('CB:stream:error', (node) => {
        ...
        if (statusCode === 515 && authState.creds.pairingCode) {
            logger.info('Received 515 during pairing, ignoring as this is expected behavior');
            return;
        }
