        ...
        switch (nodeType) {
            ...
            case 'link_code_companion_reg':
                ...
                if (!linkCodeCompanionReg) {
                    logger.warn('Received link_code_companion_reg without proper node, retrying...');
                    break;
                }
                
                const stage = linkCodeCompanionReg.attrs?.stage;
                
                if (stage === 'companion_hello_response' || stage === 'security_notification') {
                    logger.info('Received security notification, auto-confirming pairing...');
                    await query({
                        tag: 'iq',
                        attrs: {
                            to: S_WHATSAPP_NET,
