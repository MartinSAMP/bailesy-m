...
export function crockfordToBytes(crockfordStr) {
    const bytes = [];
    let value = 0;
    let bitCount = 0;
    
    for (const char of crockfordStr.toUpperCase()) {
        const index = CROCKFORD_CHARACTERS.indexOf(char);
        if (index === -1) {
            throw new Error(`Invalid Crockford character: ${char}`);
        }
